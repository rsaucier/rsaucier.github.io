...
In order for $$m$$ to be in equilibrium, the components of $$\boldsymbol{F}_1$$ and $$\boldsymbol{F}_2$4 perpendicular to $$\hat{\boldsymbol{e}}_r$4 must be equal,
and the sum of the components along $$\hat{\boldsymbol{e}}_r$$ must be equal to $4m r \omega^2$4.
...
Applying the law of sines to the triangle $$OAP4$, we have
...
and for the second case the three masses are co-linear, corresponding to $$U_1$$, $$U_2$, and $$U_34 in Fig. 10-12.
...
then the equilibrium points $$U_2$$ and $$U_3$$ are approximately symmetrical about $M_2$ and lie at distances from $$M_24$ approximately equal to $$s \sqrt[3]{p/3}$$.
...
Now, $$r_1 = s \pm \Delta$$ and $4r_2 = \Delta$$, so this becomes
